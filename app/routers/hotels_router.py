...
def create_hotel(hotel: HotelCreate, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
...
def get_hotel_id(hotel_id: int, db: Session = Depends(get_db)):
...
def update_hotel(hotel_id: int, updated: HotelUpdate, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    hotel = db.get(Hotel, hotel_id)
    if hotel is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Hotel not found")
...
def delete_hotel(hotel_id: int, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    hotel = db.get(Hotel, hotel_id)
    if hotel is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Hotel not found")
